...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    @weakify(self);
    [[[cell.editBtn rac_signalForControlEvents:UIControlEventTouchUpInside]takeUntil:cell.rac_prepareForReuseSignal]subscribeNext:^(id x){
        WSYAddAddressViewController *vc = [WSYAddAddressViewController new];
...
- (UITableView *)tableView
{
    if (_tableView == nil) {
        if (iphoneX) {
            _tableView = [[UITableView alloc] initWithFrame:(CGRect){0, 88, kScreenWidth, kScreenHeight - 160} style:UITableViewStylePlain];
        } else {
            _tableView = [[UITableView alloc] initWithFrame:(CGRect){0, 64, kScreenWidth, kScreenHeight - 114} style:UITableViewStylePlain];
        }
